 ...
 *
 * With this on, the app launches straight into the decompression session at
 * `/session` and the old arrival ritual is suppressed — the check-in and the
 * breathing circle in `EntryFlow` are the session's own first two screens done
 * a second time, and hitting both in a row is just confusing.
 *
 * The tabs are untouched and still mount underneath. Flip this to `false` to
 * get the previous launch behaviour back exactly as it was; delete this
 * constant and the two branches below once the pivot is decided either way.
 */
const LAUNCH_INTO_SESSION: boolean = true;
...
function RootNavigator() {
  ...
  //
  // One file, because Google ships Patrick Hand in one weight — and RN can't
  // synthesize the others for a custom family, so there is nothing to be gained
  // by asking. See the note on `Fonts` in `constants/theme.ts` for what the app
  // does for emphasis instead. The name comes from the package rather than
  // being retyped: it is also what `constants/theme` references, and a typo in
  // either place is silent.
  const [fontsLoaded, fontError] = useFonts({ PlaypenSans_400Regular });
  ...
  // Without a session — anonymous sign-in failed, or the device is offline —
  // there is nothing per-user to fetch, so this settles immediately and the
  // screens fall back to the public artifact.
  const userId = session?.user.id;
  // Tracked per user rather than as a bare flag, so the state is only ever set
  // from a callback — and so a new anonymous account minted after a lost
  // refresh token can't inherit the previous one's "already warm".
  ...
  // Hold the splash until the session has resolved — on a first launch that
  // includes minting the anonymous account, and a screen that mounted before it
  // landed would fire its per-user queries with no `auth.uid()` and get nothing
  // — and until the warm-up lands, so what's underneath is finished rather than
  // filling in.
  ...
  // Sent once the splash has actually lifted, so the navigator is mounted and
  // the session's first screen is what appears out of the fade rather than the
  // tabs briefly showing through. `replace`, so there is no tab screen sitting
  // behind the session for a stray gesture to reach.
  useEffect(() => {
    if (!LAUNCH_INTO_SESSION || !isRevealed) return;
    ...
    <ThemeProvider value={navigationTheme}>
      ...
      <AppReadyContext value={isRevealed}>
        {/* Header titles in the app's own face rather than the system's.
            ...
            22pt matches `sectionTitle`: Patrick Hand has short caps relative
            to its body, so a header title needs a little more than the 20 the
            sans wanted to still outrank the headings on the page below it.
        ...
        <Stack
          ...
          {/* The decompression session. A whole flow rather than a screen: it
              has its own stack, its own in-memory state, and no header or back
              button, because it runs forward only.

              Nothing in the app links here yet — it is reachable at `/session`
              (or `reground://session`) while it is being reviewed. See
              `src/app/session/_layout.tsx`. */}
        ...
        {/* The arrival ritual — a check-in, then a breath for whoever needs one
            ...
            Gated on `isRevealed` so the question appears as the splash lifts
            rather than being answered underneath it — and on nothing else, now
            that there is no sign-in screen for it to cover. The `useState` is
            what makes this once per cold launch: it resets on a cold start and
            survives a backgrounding, which is the line we want. */}
        {!LAUNCH_INTO_SESSION && isRevealed && !hasEntered ? (
